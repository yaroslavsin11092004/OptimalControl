...
net::awaitable<void> AdamApi::call_set_global_params(double learning_rate, std::vector<double> left_edge, std::vector<double> right_edge, int epochs)
{
	try 
	{
		json req_json;
		...
		req_json["left_edge"] = std::move(left_edge);
		req_json["right_edge"] = std::move(right_edge);
...
net::awaitable<std::vector<double>> AdamApi::call_adam(std::vector<double> params)
{
	try 
	{
		json req_json;
		req_json["params"] = std::move(params);
		auto resolver = tcp::resolver(co_await net::this_coro::executor);
		auto socket = tcp::socket(co_await net::this_coro::executor);
		auto endpoint = co_await resolver.async_resolve(this->host, this->port, net::use_awaitable);
		co_await net::async_connect(socket, endpoint, net::use_awaitable);
		Request req{http::verb::post, "/optimize_adam",11};
		req.body() = req_json.dump();
		req.set(http::field::content_type, "application/json");
		req.prepare_payload();
		co_await http::async_write(socket, req, net::use_awaitable);
...
void AdamApi::set_global_params(double learning_rate, std::vector<double> left_edge, std::vector<double> right_edge, int epochs)
{
	try 
	{
		...
		[self = shared_from_this(), result_promise = std::move(result_promise), learning_rate, left_edge = std::move(left_edge), right_edge = std::move(right_edge), epochs] mutable -> net::awaitable<void>
		{
			try 
			{
				co_await self->call_set_global_params(learning_rate, std::move(left_edge), std::move(right_edge), epochs);
...
std::vector<double> AdamApi::adam(std::vector<double> params)
{
	try 
	{
		...
		auto result_future = result_promise.get_future();
		net::co_spawn(*this->ioc, 
		[self = shared_from_this(), params = std::move(params), result_promise = std::move(result_promise)] mutable -> net::awaitable<void>
		{
			try 
			{
				std::vector<double> result = co_await self->call_adam(std::move(params));
		...
		net::detached);
		return std::move(result_future.get());
	}
	catch(const std::exception& e)
	{
		throw std::runtime_error(e.what());
